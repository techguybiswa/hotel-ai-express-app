 ...
 */
class ChatPromptValue extends prompt_values_js_1.BasePromptValue {
    static lc_name() {
...
class MessagesPlaceholder extends BaseMessagePromptTemplate {
    ...
    validateInputOrThrow(input, variableName) {
        ...
        if (Array.isArray(input)) {
            isInputBaseMessage = input.every((message) => (0, index_js_2.isBaseMessage)(message));
        }
        else {
            isInputBaseMessage = (0, index_js_2.isBaseMessage)(input);
        }
...
class ChatMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new index_js_2.ChatMessage(await this.prompt.format(values), this.role);
    }
...
class HumanMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new index_js_2.HumanMessage(await this.prompt.format(values));
    }
...
class AIMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new index_js_2.AIMessage(await this.prompt.format(values));
    }
    static fromTemplate(template) {
        return new this(prompt_js_1.PromptTemplate.fromTemplate(template));
    }
...
class SystemMessagePromptTemplate extends BaseMessageStringPromptTemplate {
    ...
    async format(values) {
        return new index_js_2.SystemMessage(await this.prompt.format(values));
    }
    static fromTemplate(template) {
        return new this(prompt_js_1.PromptTemplate.fromTemplate(template));
    }
...
function _coerceMessagePromptTemplateLike(messagePromptTemplateLike) {
    if (_isBaseMessagePromptTemplate(messagePromptTemplateLike) ||
        (0, index_js_2.isBaseMessage)(messagePromptTemplateLike)) {
        return messagePromptTemplateLike;
    }
    const message = (0, index_js_2.coerceMessageLikeToMessage)(messagePromptTemplateLike);
    ...
    else if (index_js_2.ChatMessage.isInstance(message)) {
    ...
    constructor(input) {
        ...
        if (this.validateTemplate) {
            ...
            for (const promptMessage of this.promptMessages) {
                ...
                if (promptMessage instanceof index_js_2.BaseMessage)
                    continue;
